...
class Isolator:
    ...
    def from_file(cls, model, input_file: str):
        ...
        instance.species = torch.tensor(
            instance.structure.numbers).unsqueeze(0).clone().detach()
        instance.coordinates = torch.tensor(
            instance.structure.positions).double().unsqueeze(0).clone().detach()
        instance.symbols = np.asarray(
            instance.structure.get_chemical_symbols()).astype(str)
        return instance
        ...
        * Converts the obabel mol2 object into a rdkit.Chem molecule to print the SMILES return the molecule
        ...
        with tempfile.NamedTemporaryFile("w+") as f:
            ...
            for idx, symbol in enumerate(self.symbols):
                f.write(
                    f"{symbol} {self.numpy_coordinates[idx][0]:8.3} {self.numpy_coordinates[idx][1]:8.3} {self.numpy_coordinates[idx][2]:8.3}\n")
            f.seek(0)
            ...
            if molecule is None:
                print(
                    "WARNING: Molecule could not be generated by RDKit for given input.")
                return None
            if return_smiles:
    ...
    def process_molecule(self, bad_atoms, output_file_prefix="/home/nick/capped_", output_format='xyz'):
        ...
        for bad_atom_idx in bad_atoms:
            ...
            capped_elements = [PERIODIC_TABLE[num]
                               for num in capped_structure.numbers]
